...
void Usage();
const char *AspectDesc(Aspect Aspect);
...
int main(int ArgC, char **ArgV)
{
    ...
    do {
        ...
        *XPos1 = '\x0';
        ...
        // Work out frame ratio and aspect
        FrameRatio = (double)DstW / (double)DstH;
        if (FrameRatio == 1.0)
            FrameAspect = Square;
        else if (FrameRatio < 1.0)
        ...
        if (Verbose) {
            ...
            if (EditDir) {
                fprintf(stdout, " (edit directory %s)", EditDir);
            }

            fprintf(stdout, ", size %dx%d", DstW, DstH);

            if (Ext != NULL) {
                fprintf(stdout, ", new ext %s", Ext);
            }
...
int ScanDir(char *SrcDir, char *EditDir, char *DstDir)
{
    ...
    if (SkipDir(SrcDir)) {
        if (Verbose) {
            fprintf(stdout, "Skipping directory %s...\n", SrcDir);
        }
    }
    else {
        if (Verbose) {
...
int ProcessDirEnt(char *File, char *SrcDir, char *EditDir, char *DstDir)
{
    ...
    if (Result != ERR_OK) {
        ...
        if (S_ISREG(Mode)) {
            ...
            if (Verbose) {
                fprintf(stderr, "Skipping %s", FilePath);
            }
        }
    }
        ...
        if (lstat(DstPath, &DstStatBuf) == 0) {
            ...
            if (S_ISREG(DstStatBuf.st_mode)) {
                if (FileTime(&DstStatBuf) > FileTime(&SrcStatBuf)) {
                    if (Verbose) {
                        fprintf(stdout, "%s newer than %s. skipping\n", DstPath, SrcPath);
                    }
                    break;
                }
            }
...
int ProcessFile(char *SrcPath, char *DstPath)
{
    ...
    do
    {
        ...
        case TopRightOrientation: // 2, flipped horizontally
            if (Verbose) {
                fprintf(stdout, "  flipping horizontally to normalise orientation\n");
            }
            Picture.flop();
            break;
        ...
        case BottomRightOrientation: // 3, rotated 180
            if (Verbose) {
                fprintf(stdout, "  flipping horizontally and vertically (180 rotation) to normalise orientation\n");
            }
            Picture.flip();
            Picture.flop();
            break;
        ...
        case BottomLeftOrientation: // 4, flipped vertically
            if (Verbose) {
                fprintf(stdout, "  flipping vertically to normalise orientation\n");
            }
            Picture.flip();
            break;
        ...
        case LeftTopOrientation: // 5, - orient changed
            if (Verbose) {
                fprintf(stdout, "  rotating 270 to normalise orientation\n");
            }
            Picture.rotate(270);
            Picture.flip();
            break;
        ...
        case RightTopOrientation: // 6, rotated 90 - orient changed
            if (Verbose) {
                fprintf(stdout, "  rotating 90 to normalise orientation\n");
            }
            Picture.rotate(90);
            break;
        ...
        case RightBottomOrientation: // 7, - orient changed
            if (Verbose) {
                fprintf(stdout, "  rotating 90 and flipping vertically to normalise orientation\n");
            }
            Picture.rotate(90);
            Picture.flip();
            break;
        ...
        if (Verbose) {
            fprintf(stdout, "  Size=%lux%lu (%.1f MP), Resolution=%.0fx%.0f, Aspect=%s, Type=%s\n",
                    Picture.columns(), Picture.rows(),
                    ((double)Picture.columns() * (double)Picture.rows()) / 1000000.0,
                    Picture.xResolution(), Picture.yResolution(),
                    AspectDesc(ImageAspect),
                    Picture.magick().c_str());
        }

        // Check image aspect is compatible
        if (FrameAspect != Square && ImageAspect != Square && ImageAspect != FrameAspect) {
            fprintf(stdout, "  Skipping %s (image is wrong aspect for frame)\n", SrcPath);
            break;
        }

        if (Verbose) {
            fprintf(stdout, "  Outputting to file %s\n", DstPath);
        }
        ...
        if (ImageRatio != FrameRatio) {
            ...
            if (ImageRatio < FrameRatio) {
                ...
                size_t TargetSize = (size_t) ((double) Picture.columns() / FrameRatio);

                size_t MinSize = (size_t) ((double) Picture.rows() * MIN_CROP);
                if (TargetSize < MinSize) {
                    fprintf(stdout, "  Skipping %s (image aspect ratio is too different %lux%lu -> %lux[%lu, min %lu])\n",
                        SrcPath, Picture.columns(), Picture.rows(), Picture.columns(), TargetSize, MinSize);
                    break;
                }
                ...
                size_t TargetSize = (size_t) ((double) Picture.rows() * FrameRatio);

                size_t MinSize = (size_t) ((double) Picture.columns() * MIN_CROP);
                if (TargetSize < MinSize) {
                    fprintf(stdout, "  Skipping %s (image aspect ratio is too different %lux%lu -> [%lu, min %lu]x%lu)\n",
                        SrcPath, Picture.columns(), Picture.rows(), TargetSize, MinSize, Picture.rows());
                    break;
                }
            ...
            if (Verbose) {
                fprintf(stdout, "  Cropping to %lux%lu at %lux%lu\n", Geom.width(), Geom.height(), Geom.xOff(), Geom.yOff());
            }
...
bool SkipDir(char *Dir)
{
    ...
    if (strcmp(Dir, DstDir) == 0) {
        Skip = true;
    }
    else {
        sprintf(TokenPath, "%s/.FrameSkip", Dir);

        if (FileMode(TokenPath, &Mode) == 0) {
            Skip = true;
        }
    }
